...
class CanvasRenderer extends BaseRenderer {
  ...
  ctxTransform(props) {
    ...
    let cProps = this.contextData.cTr.props;
    this.transformMat.transform(cProps[0], cProps[1], cProps[2], cProps[3], cProps[4], cProps[5], cProps[6], cProps[7], cProps[8], cProps[9], cProps[10], cProps[11], cProps[12], cProps[13], cProps[14], cProps[15]);
    this.contextData.cTr.cloneFromProps(this.transformMat.props);
    let trProps = this.contextData.cTr.props;
  ...
  ctxOpacity(op) {
    ...
    if (!this.renderConfig.clearCanvas) {
      const globalAlpha = this.canvasContext.globalAlpha * (op < 0 ? 0 : op);
  ...
  save(actionFlag) {
    ...
    let props = this.contextData.cTr.props;
    if (this.contextData._length <= this.contextData.cArrPos) {
      this.contextData.duplicate();
    }
    let i;
    let arr = this.contextData.saved[this.contextData.cArrPos];
    for (i = 0; i < 16; i += 1) {
      arr[i] = props[i];
    }
  ...
  restore(actionFlag) {
    ...
    let i;
    let arr = this.contextData.cTr.props;
    for (i = 0; i < 16; i += 1) {
      arr[i] = popped[i];
    }
  ...
  updateContainerSize() {
    ...
    if (this.renderConfig.preserveAspectRatio.indexOf('meet') !== -1 || this.renderConfig.preserveAspectRatio.indexOf('slice') !== -1) {
      ...
      let fillType = par[1] || 'meet';
      let pos = par[0] || 'xMidYMid';
      let xPos = pos.substr(0, 4);
      let yPos = pos.substr(4);
  ...
  destroy() {
    let i;
    let len = this.layers ? this.layers.length : 0;
    for (i = len - 1; i >= 0; i -= 1) {
      if (this.elements[i]) {
